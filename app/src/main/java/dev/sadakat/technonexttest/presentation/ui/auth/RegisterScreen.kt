...
fun RegisterScreen(
    ...
    val isValidEmail = remember(email) { PasswordUtils.isValidEmail(email) }
    val isValidPassword = remember(password) { PasswordUtils.isValidPassword(password) }
    ...
    val onConfirmPasswordChange =
        remember { { newConfirmPassword: String -> confirmPassword = newConfirmPassword } }
    val togglePasswordVisibility = remember { { passwordVisible = !passwordVisible } }
    val toggleConfirmPasswordVisibility =
        remember { { confirmPasswordVisible = !confirmPasswordVisible } }
    val performRegister = remember { { onRegister(email, password, confirmPassword) } }
    ...
    Column(
        ...
        OutlinedTextField(
            ...
            supportingText = {
                if (password.isNotBlank() && !isValidPassword) {
                    Text("Password must be 8+ chars with uppercase, lowercase, number, and special character")
                }
            })
        ...
        Button(
            ...
            modifier = Modifier.fillMaxWidth(),
            enabled = !isLoading && isValidEmail && isValidPassword && passwordsMatch
        ) {
            if (isLoading) {
        ...
        TextButton(
            onClick = onNavigateToLogin, enabled = !isLoading
        ) {
